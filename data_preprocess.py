...
class Data_preprocess():
    def __init__(self, batch_size=64, data_type='train'):
        ...
        if data_type == 'train':
            self.data_path = '../data_exp4/data_exp4_train.txt'
        elif data_type == 'dev':
            self.data_path = '../data_exp4/data_exp4_val.txt'
            self.load_data_map()
        elif data_type == 'test':
            self.data_path = '../data_exp4/data_exp4_test.txt'
            self.load_data_map()
    ...
    def load_data(self):
        ...
        with open('result.txt', 'a', encoding='utf-8') as f:
            f.write(f'{self.data_type} data: {len(self.data)}')
            f.write(f'vocab size: {self.input_size}')
            f.write(f'unique tag: {len(self.tags_map)}')
            f.write('*' * 50)
